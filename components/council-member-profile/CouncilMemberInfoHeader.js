          ...
          <div className="flex items-center">
            {socialInfo.twitter && (
              <button
                ...
                onClick={() =>
                  openWindow(
                    `https://twitter.com/${socialInfo.twitter.substring(
                      socialInfo.twitter.lastIndexOf("/") + 1,
                      socialInfo.twitter.length
					)}` // Tohandle users who have saved an entire url for twitter instead of just the handle in their identity
                  ) // like https://twitter.com/FurqanAhmed instead of just FurqanAhmed
                }
              >
            ...
            {socialInfo.web && (
              <button
                className="mr-4 text-sm flex items-center hover:underline"
                onClick={() => openWindow(`http://${socialInfo.web}`)}
              >
